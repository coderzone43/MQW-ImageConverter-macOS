...
public struct ARParser: MarkupVisitor {
    let baseFontSize: CGFloat = 15.0
    let highlightr = Highlighter()
    let newLineFontSize = 12.0
    ...
    mutating func parserResults(from document: Document) -> [ParserResult] {
        ...
        func appendCurrentAttrString() {
            if !currentAttrString.string.isEmpty {
                let currentAttrStringToAppend = currentAttrString
        ...
        document.children.forEach { markup in
            ...
            if let codeBlock = markup as? CodeBlock {
                ...
                let attrStringToAppend = attrString
    ...
    mutating public func visitText(_ text: Text) -> NSAttributedString {
        return NSAttributedString(
            ...
            attributes: [
                .font: text.plainText == "No Response".localized() ?
                NSFont.systemFont(ofSize: baseFontSize, weight: .regular).apply(newTraits: .italic) :
                NSFont.systemFont(ofSize: baseFontSize, weight: .regular),
                .foregroundColor: text.plainText == "No Response".localized() ? NSColor.secondaryLabelColor : NSColor.textColor
            ]
        )
    }
    ...
    public mutating func visitCodeBlock(_ codeBlock: CodeBlock) -> NSAttributedString {
        ...
        func result() -> NSAttributedString {
            if isStreaming {
                let result = NSMutableAttributedString(string: codeBlock.code, attributes: [.font: NSFont.monospacedSystemFont(ofSize: baseFontSize - 1.0, weight: .regular), .foregroundColor: NSColor.secondaryLabelColor])
                ...
                if NSApp.effectiveAppearance == NSAppearance(named: .darkAqua) {
                    highlightr?.setTheme("atom-one-dark")
                } else {
                    highlightr?.setTheme("atom-one-light")
                }
                let result: NSMutableAttributedString = NSMutableAttributedString(attributedString: highlightr?.highlight(codeBlock.code) ?? NSAttributedString(""))
    ...
    mutating public func visitUnorderedList(_ unorderedList: UnorderedList) -> NSAttributedString {
        ...
        for listItem in unorderedList.listItems {
            ...
            listItemAttributes[.foregroundColor] = NSColor.textColor
    ...
    mutating public func visitBlockQuote(_ blockQuote: BlockQuote) -> NSAttributedString {
        ...
        for child in blockQuote.children {
            ...
            quoteAttributedString.addAttribute(.foregroundColor, value: NSColor.secondaryLabelColor)
...
extension NSMutableAttributedString {
    func applyEmphasis() {
        enumerateAttribute(.font, in: NSRange(location: 0, length: length), options: []) { value, range, stop in
            ...
            let newFont = font.apply(newTraits: .italic, newPointSize: 16)
            addAttribute(.font, value: newFont, range: range)
            addAttribute(.foregroundColor, value: NSColor.textColor)
        }
    }
    ...
    func applyStrong() {
        enumerateAttribute(.font, in: NSRange(location: 0, length: length), options: []) { value, range, stop in
            ...
            let newFont: NSFont = font.apply(newTraits: .bold, newPointSize: 16)
            addAttribute(.font, value: newFont, range: range)
            addAttribute(.foregroundColor, value: NSColor.textColor, range: range)
        }
    }
    
    func applyLink(withURL url: URL?) {
        addAttribute(.foregroundColor, value: NSColor.linkColor)
    ...
    func applyBlockquote() {
        addAttribute(.foregroundColor, value: NSColor.secondaryLabelColor)
    }
